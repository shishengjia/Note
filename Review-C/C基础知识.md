...
**变量**的实质是代表“内存中的某个存储单元”<br>
**指针变量**是用来存储地址的，而一般变量是存储数值的。指针变量可指向任意一种数据类型，但不管它指向的数据占用多少字节，一个指针变量占用4个字节。<br>
格式为:**类型名 \*指针变量名**<br>
二维指针int \*\*p,可以理解为基类型为int \*类型，及指针的指针。

\*p++和(\*p)++之间的差别:\*p++是地址变化，而(\*p)++是指针变量所指的数据变化。一个指针变量加一个整数不是简单的数学相加，而是连续移动若干地址。
当两个指针指向同一数组时，它们之间可以比较大小进行运算。
...
结构体允许嵌套,一个结构体变量所占字节数为其所有成员所占字节数之和。<br>
在赋值时，按结构体每个成员在结构体中的顺序一一对应赋初值，不允许跳过前面的成员给后面的成员赋初值，若只给前面几个成员赋初值，后面的成员的字符和整形
将会赋默认值0.
...
//给结构体定义别明struct Student->Student
...
int main() {
    //初始化结构体数组
    Student arr[3] = {{"Tom",22},{"Jack",23},{"Boen",34}};
    //指针数组，分别保存每个结构体的地址
    Student *p[3] = {arr,&arr[1],&arr[2]};
    //输出
    for(int i=0;i<3;i++)
...
{ <br>
  ...
  结构体和联合体的区别:结构体变量中的成员各自占用自己的存储空间，而联合体中的所有成员占有同一个存储空间.<br>
  共用体所占字节数是所有成员中字节数最大的那个.<br>
  联合体的定义与引用和结构体类似
